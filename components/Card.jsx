...
export default function Card({imgURL,cardData,setMain,main}) {
  ...
  return (
    <div  className='border-2 relative min-w-[10rem] w-[10rem] h-[15rem] mx-4 flex items-end overflow-hidden'>
      <img className='absolute top-0 left-0 right-0' src={imgURL} alt="" />
      <div className='w-full h-[4.6rem] bg-gray-100  z-30'>
    ...
    <p>{(data.averageRating * 0.1).toFixed(1)}</p>
          ...
          <AiOutlineInfoCircle className='w-5 h-5 my-auto cursor-pointer' />
        </div>
        <h4 ref={nameH4} style={{lineHeight:"24px"}} >{lines3?`${name.substring(0,29)}...`:name}</h4>
      </div>
    </div>
  )
}
